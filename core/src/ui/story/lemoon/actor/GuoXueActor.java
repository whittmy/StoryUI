...
public class GuoXueActor extends Actor{
	...
	public GuoXueActor(Configer cfg){
		...
		mTextureRegion = MyGame.mCfg.getMainAtlas().findRegion("item_guoxue");
		...
		input = new InputListener(){
			...
			public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
				...
				//Configer.mylog("", "GuoXue up x="+x+", y="+y);
			...
			public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
				// TODO Auto-generated method stub
				Configer.mylog("", "GuoXue down x="+x+", y="+y);
				if(mTimer != null){
					mTimer.cancel();
					mTimer = null;
				}
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		//Configer.mylog("", getWidth()+","+getHeight()+"===,"+mW+","+mH);
