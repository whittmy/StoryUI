...
public class MathActor extends Actor{
	//30.6,17.5=>30.6, 32-17.5-1=13.5
	AtlasRegion mTextureRegion;
	float mPosx=30.8f, mPosy=13.3f;
	float mW,mH;
	...
	Timer mTimer = null;
	InputListener input;
	public MathActor(Configer cfg){
		super();
		//
		
		mTextureRegion = MyGame.mCfg.getMainAtlas().findRegion("item_math");
		...
		input = new InputListener(){
			...
			public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
				...
				//Configer.mylog("", "Math up x="+x+", y="+y);
//				MyMsg msg = new MyMsg();
...
//				MyGame.mCfg.game.notify(msg);
			...
			public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
				// TODO Auto-generated method stub
				Configer.mylog("", "Math down x="+x+", y="+y);
