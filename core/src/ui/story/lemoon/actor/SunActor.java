...
public class SunActor extends Actor{
	...
	public SunActor(Configer cfg){
		...
		mSunFrams[0] = MyGame.mCfg.getMainAtlas().findRegion("sun1");
		mSunFrams[1] = MyGame.mCfg.getMainAtlas().findRegion("sun2");
		mSunFrams[2] = MyGame.mCfg.getMainAtlas().findRegion("sun3");
		mSunFrams[3] = MyGame.mCfg.getMainAtlas().findRegion("sun4");
		mSunFrams[4] = MyGame.mCfg.getMainAtlas().findRegion("sun5");		
	...
	void computePosition(){
		lastangle  = (float) ((MyGame.mCfg.mHour*3600+MyGame.mCfg.mMin*60+MyGame.mCfg.mSec - 21600)*0.00324074074074074074074074074074) + 20; //前半部分计算相对于6点的偏移度数
		//Configer.mylog("", "angle:"+lastangle);
		if(lastangle > 160){
			lastangle = 160;
		}
		...
		//Configer.mylog("", String.format("%d:%d:%d, x:%f,y:%f", MyGame.mCfg.mHour, MyGame.mCfg.mMin,MyGame.mCfg.mSec,mPosSunX,mPosSunY));
	...
	void checkState(){
		if(MyGame.mCfg.mBDayMode){
			...
			//计算太阳运行偏移度数
			if(MyGame.mCfg.mBhadsettime && /*MyGame.mCfg.mSec-lastsec > 0*/MyGame.mCfg.mSec != lastsec){
				computePosition();
				lastsec = MyGame.mCfg.mSec;
			}
